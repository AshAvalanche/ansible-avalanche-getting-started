...
curl -s -X POST --data '{
  ...
  "params": { "chain": "P" }
}' -H 'content-type:application/json;' http://192.168.10.11:9650/ext/info
```
...
key_1=$(curl -s -X POST -H 'content-type:application/json;' --data "$data" http://192.168.10.11:9650/ext/bc/P | jq -r '.result.address')
key_2=$(curl -s -X POST -H 'content-type:application/json;' --data "$data" http://192.168.10.11:9650/ext/bc/P | jq -r '.result.address')
...
The node `validator01-local` exposes AvalancheGo APIs on it's public IP: you can query any [Avalanche API](https://docs.avax.network/build/avalanchego-apis/) at `192.168.10.11:9650` from your terminal. For example, to check if the P-Chain is done bootstrapping:
...
3. The notebook issues 2 transactions to transfer 1 AVAX from the X-Chain to the C-Chain and you can see the `tx` module output format:
   ...
   xchain_transac_res:
     blockchain: X
     changed: true
     endpoint: http://192.168.10.11:9650/ext/bc/X
     failed: false
     num_retries: 0
     response:
       id: 1
